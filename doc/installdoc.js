...
function encode(string, exclude, keepEscaped) {
  ...
  for (let i4 = 0, l3 = string.length; i4 < l3; i4++) {
    ...
    if (code2 >= 55296 && code2 <= 57343) {
      if (code2 >= 55296 && code2 <= 56319 && i4 + 1 < l3) {
        ...
        if (nextCode >= 56320 && nextCode <= 57343) {
          result2 += encodeURIComponent(string[i4] + string[i4 + 1]);
          i4++;
          continue;
        }
      }
    ...
    result2 += encodeURIComponent(string[i4]);
  }
  return result2;
}
...
Renderer.prototype.renderAttrs = function renderAttrs(token) {
  ...
  for (i4 = 0, l3 = token.attrs.length; i4 < l3; i4++) {
    result2 += " " + escapeHtml(token.attrs[i4][0]) + '="' + escapeHtml(token.attrs[i4][1]) + '"';
  }
  return result2;
};
...
Renderer.prototype.renderInline = function(tokens, options, env) {
  ...
  for (let i4 = 0, len = tokens.length; i4 < len; i4++) {
    ...
    if (typeof rules[type] !== "undefined") {
      result2 += rules[type](tokens, i4, options, env, this);
    } else {
      result2 += this.renderToken(tokens, i4, options);
    }
  }
  return result2;
};
Renderer.prototype.renderInlineAsText = function(tokens, options, env) {
  ...
  for (let i4 = 0, len = tokens.length; i4 < len; i4++) {
    switch (tokens[i4].type) {
      case "text":
        result2 += tokens[i4].content;
        break;
      case "image":
        result2 += this.renderInlineAsText(tokens[i4].children, options, env);
        break;
      case "html_inline":
      case "html_block":
        result2 += tokens[i4].content;
...
Renderer.prototype.render = function(tokens, options, env) {
  ...
  for (let i4 = 0, len = tokens.length; i4 < len; i4++) {
    const type = tokens[i4].type;
    if (type === "inline") {
      result2 += this.renderInline(tokens[i4].children, options, env);
    } else if (typeof rules[type] !== "undefined") {
      result2 += rules[type](tokens, i4, options, env, this);
    } else {
      result2 += this.renderToken(tokens, i4, options, env);
    }
  }
  return result2;
};
  ...
  for (let i4 = 0; i4 < tokens.length; i4++) {
    ...
    OUTER:
      while (pos < max) {
        ...
        let lastChar = 32;
        if (t3.index - 1 >= 0) {
          lastChar = text3.charCodeAt(t3.index - 1);
        } else {
          for (j3 = i4 - 1; j3 >= 0; j3--) {
...
if (mount) {
  B(/* @__PURE__ */ u2(Doc, { document: result, title: "install" }), mount);
}
